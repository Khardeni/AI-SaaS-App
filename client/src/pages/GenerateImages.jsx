...
const GenerateImages = () => {
  ...
  const onsubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate an image in the style of ${selectedStyle} about "${input}".`;
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex flex-col lg:flex-row items-start gap-4 text-slate-700'>
      ...
      <form onSubmit={onsubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200 '>
        ...
        <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 rounded-lg mt-6 text-sm cursor-pointer hover:opacity-90 transition-all duration-200'>
